...
var generatePassword = function() {
  // Declare variables that the confirm prompts will add or not add to the pool of characters from which the password will be created.
  var uppers = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  var lowers = uppers.toLowerCase();
  var numbers = "1234567890";
  var specials = "~!@#$%^&*()_+-=[]{}|;:.,?><"

  // Declare the variable that contains the number of characters that the password will contain--so we know how many times to choose a character at random from the pool to generate the password.
  ...
  // If no character types have been chosen, this while statement will alert the user to their failure to choose at least one character type and then go through the confirms again.
  ...
  // Build a single pool of characters, based on which character types the user confirmed above, from which the password will be generated.
  var passwordPool;
  // if a confirm variable was confirmed, then add the corresponding string of characters to the password pool.
  if (confirmUppers) {
  ...
  // the string that makes up the passwordPool variable at this point still contains the string "undefined" from when the variable was first declared. Remove "undefined" from the passwordPool string (especially important if lowers are not confirmed).
  passwordPool = passwordPool.slice(9);
  
  // Declare password1 variable and give it its first character--eventually it will be returned to the 'password' variable and then written into the page.
  ...
  for (i = 1; i < charLength; i++) {
    // each character of the password is generated by adding a character chosen at random from the passwordPool string to the password1 string so far generated.
